...
touch /net/rcc/dev/shmem/backupplus_gndb.mib

# Setup cleanup trap for proper lock file removal
cleanup_backupplus_gndb() {
	rm -f /net/rcc/dev/shmem/backupplus_gndb.mib 2>/dev/null
}
trap cleanup_backupplus_gndb EXIT TERM INT
