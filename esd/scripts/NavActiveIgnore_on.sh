...
touch /net/rcc/dev/shmem/NavActiveIgnore_on.mib

# Setup cleanup trap for proper lock file removal
cleanup_NavActiveIgnore_on() {
	rm -f /net/rcc/dev/shmem/NavActiveIgnore_on.mib 2>/dev/null
}
trap cleanup_NavActiveIgnore_on EXIT TERM INT
...
on -f rcc /net/mmx/fs/sda0/apps/installjava -navignoreon
sleep 3
